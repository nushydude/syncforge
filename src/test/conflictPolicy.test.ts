...
function file(path: string, size: number, modifiedSecs: number): FileEntry {
...
describe("conflictPolicy", () => {
  ...
  it("classifies delete vs new using snapshot", () => {
    ...
    expect(classifyPathChange(file("gone.txt", 1, 1), undefined, snap)).toBe(
      "deletedOnRight",
    );
    expect(classifyPathChange(undefined, file("gone.txt", 1, 1), snap)).toBe(
      "deletedOnLeft",
    );
    expect(
