...
import { deleteSnifferItem, renameSnifferItem, scanFolderSizes, showSnifferItemProperties } from "../../api/sniffer";
...
function buildTreemap(entries: FolderSizeEntry[]): TreemapTile[] {
  ...
  function layout(
    ...
    if (group.length === 1) {
      tiles.push({ entry: group[0], left, top, width, height, tone: tiles.length % 6 });
      return;
    }
    ...
    const firstRatio = first.reduce((sum, entry) => sum + entry.size, 0) / groupTotal;
    if (width >= height) {
    ...
    <section className="sniffer-file-view" aria-label="Largest files">
      <header className="sniffer-map-header">
        ...
        <div className="sniffer-file-view-actions">
          <span className="sniffer-file-count">{expandedFiles ? `Showing all ${files.length.toLocaleString()}` : `Top ${visibleFiles.length} of ${files.length.toLocaleString()}`}</span>
          {onShowInMap && <button type="button" onClick={onShowInMap}>Show files in map</button>}
        </div>
      </header>
      <div className="sniffer-file-list">
        {visibleFiles.map((entry) => (
          <div className={`sniffer-file-row ${contextEntryPath === entry.path ? "context-selected" : ""}`} key={entry.path} title={entry.path} onContextMenu={(event) => onEntryContextMenu?.(event, entry)}>
            <div className="sniffer-file-row-label">
              <strong>{entry.name}</strong>
              <span>{formatBytes(entry.size)}</span>
            </div>
            <div className="sniffer-file-bar-track" aria-hidden="true">
              <div className="sniffer-file-bar" style={{ width: `${Math.max(2, (entry.size / maxSize) * 100)}%` }} />
            </div>
          </div>
        ))}
        {otherSize > 0 && !expandedFiles && (
          <button type="button" className="sniffer-file-row other-file-row" onClick={() => setExpandedFiles(true)} title="Expand the remaining files in this view">
            <div className="sniffer-file-row-label">
              <strong>Other files <span className="sniffer-inline-hint">· click to expand</span></strong>
              <span>{formatBytes(otherSize)}</span>
            </div>
            <div className="sniffer-file-bar-track" aria-hidden="true">
              <div className="sniffer-file-bar" style={{ width: `${Math.max(2, (otherSize / maxSize) * 100)}%` }} />
            </div>
          </button>
        )}
      </div>
...
export function FolderSnifferView() {
  ...
  const [scanProgress, setScanProgress] = useState<SnifferProgress | null>(null);
  const [contextMenu, setContextMenu] = useState<{ x: number; y: number; entry: FolderSizeEntry } | null>(null);
  const [toast, setToast] = useState("");
  const scanCache = useRef(new Map<string, FolderSizeResult>());
  const mapLimit = 40;

  function handleEntryContextMenu(event: ReactMouseEvent, entry: FolderSizeEntry) {
  ...
  async function runContextAction(action: "open" | "reveal" | "copy-name" | "copy-path" | "properties" | "rename" | "delete") {
    ...
    try {
      ...
      else if (action === "properties") await showSnifferItemProperties(systemPath);
      ...
      } else if (window.confirm(`Delete ${entry.isFolder ? "folder" : "file"} “${entry.name}”? This cannot be undone.`)) {
  ...
  useEffect(() => {
    ...
    const maxY = view.scrollTop + view.clientHeight - menu.offsetHeight - margin;
    ...
    <main className="sniffer-view" ref={snifferViewRef}>
      <header className="workspace-header sniffer-header">
        <div>
          ...
          <p>See where the space is going, then open any folder to inspect it.</p>
        </div>
        <button type="button" className="btn-primary" onClick={() => void chooseFolder()} disabled={loading}>
      ...
      {!result && !loading && !error && (
        <section className="sniffer-empty">
          <div className="sniffer-empty-icon" aria-hidden="true">◒</div>
          <h3>Find your largest folders</h3>
          <p>Select a folder to map its files and subfolders by size.</p>
          <button type="button" className="btn-primary" onClick={() => void chooseFolder()}>Scan folder</button>
        </section>
      )}

      {loading && (
        <section className="sniffer-scan-progress" role="status" aria-live="polite">
          <div className="sniffer-scan-progress-heading">
            <span className="sniffer-scan-spinner" aria-hidden="true" />
            <strong>Scanning folder… {scanProgress && scanProgress.total > 0 ? `${Math.round((scanProgress.completed / scanProgress.total) * 100)}%` : ""}</strong>
          </div>
          <p>{scanProgress && scanProgress.total > 0 ? `Completed ${scanProgress.completed.toLocaleString()} of ${scanProgress.total.toLocaleString()} top-level items.` : "Walking files and folders. Large drives may take a while."}</p>
          <div className="sniffer-progress-track" aria-label="Scan in progress">
            {scanProgress && scanProgress.total > 0 ? <div className="sniffer-progress-determinate" style={{ width: `${(scanProgress.completed / scanProgress.total) * 100}%` }} /> : <div className="sniffer-progress-indeterminate" />}
          </div>
          <small>{scanProgress?.current ? shortPath(scanProgress.current) : currentPath && shortPath(currentPath)}</small>
        </section>
      )}
      {error && <div className="sniffer-error" role="alert"><strong>Could not scan folder</strong><p>{error}</p><button type="button" onClick={() => void openFolder(currentPath, pathTrail, true)}>Retry</button></div>}

      {result && !loading && (
        <section className="sniffer-results" aria-label="Folder size visualization">
          <div className="sniffer-breadcrumb">
              ...
              <nav className="sniffer-trail" aria-label="Folder path">
                {pathTrail.map((path, index) => (
                  <span className="sniffer-trail-segment" key={path}>
                    {index > 0 && <span className="sniffer-trail-separator" aria-hidden="true">/</span>}
                    {index === pathTrail.length - 1 ? (
                      <span className="sniffer-breadcrumb-path" title={path} aria-current="location">{folderName(path)}</span>
                    ) : (
                      <button type="button" className="sniffer-trail-link" onClick={() => void openFolder(path, pathTrail.slice(0, index + 1))} title={path}>
            ...
            <div className="sniffer-breadcrumb-actions">
              {pathTrail.length > 1 && <button type="button" onClick={goUpOneFolder} aria-label="Go up one folder">Back</button>}
              {!result.path.endsWith("\\__other__") && <button type="button" onClick={() => void openFolder(result.path, pathTrail, true)}>Refresh scan</button>}
            </div>
          </div>
          <div className="sniffer-summary">
            <div><strong>{formatBytes(result.size)}</strong><span>Total size</span></div>
            <div><strong>{result.folders.toLocaleString()}</strong><span>Subfolders</span></div>
            <div><strong>{result.files.toLocaleString()}</strong><span>Files</span></div>
          </div>
          {result.folders === 0 && result.files > 0 && !showFilesInMap ? <FileSizeView result={result} onShowInMap={() => setShowFilesInMap(true)} onEntryContextMenu={handleEntryContextMenu} contextEntryPath={contextMenu?.entry.path} /> : <>
          <div className="sniffer-map-header">
            <div>
              <h3>Storage map</h3>
              <p>Larger areas represent more space. Click a folder to open it, or expand the remaining items here.</p>
            </div>
            <div className="sniffer-map-meta">
              {showingCached && <span className="sniffer-cache-badge">Cached view</span>}
              <span className="sniffer-detail-count">Largest {mapLimit} items</span>
              {result.files > 0 && <button type="button" className="sniffer-files-toggle" onClick={() => setShowFilesInMap((visible) => !visible)}>{showFilesInMap ? "Group files" : "Show files in map"}</button>}
              <div className="sniffer-legend" aria-label="Map legend">
                <span><i className="legend-swatch folder" />Folders</span>
                <span><i className="legend-swatch file" />Files</span>
              </div>
            </div>
          </div>
          {result.entries.length === 0 ? <p className="sniffer-empty-inline">This folder is empty.</p> : (
            <div className="sniffer-map">
              {treemap.map(({ entry, left, top, width, height, tone }) => (
                <div
                  ...
                  onContextMenu={(event) => {
                    if (entry.name !== "Other items" && entry.name !== "Loose files") {
                      handleEntryContextMenu(event, entry);
                    }
                  }}
                >
                  {entry.name === "Other items" ? (
                    <button type="button" className="sniffer-tile-button" onClick={toggleOtherItems} title="Expand the remaining items in this map">
                      <strong>Other items</strong><span>{formatBytes(entry.size)}</span><small>{entry.childCount.toLocaleString()} items · click to expand</small>
                    </button>
                  ) : entry.isFolder ? (
                    <button type="button" className="sniffer-tile-button" onClick={() => void openFolder(entry.path, [...pathTrail, entry.path])} title={`Open ${entry.name}`}>
                      {width > 8 && height > 12 ? <><strong>{tileLabel(entry)}</strong><span>{formatBytes(entry.size)}</span>{width > 13 && height > 17 && <small>{entry.childCount.toLocaleString()} items</small>}</> : <span className="sniffer-tile-glyph" aria-hidden="true">›</span>}
                    </button>
                  ) : <div className="sniffer-tile-content">{width > 8 && height > 12 ? <><strong>{tileLabel(entry)}</strong><span>{formatBytes(entry.size)}</span>{entry.childCount > 0 && width > 13 && height > 17 && <small>{entry.childCount.toLocaleString()} more items</small>}</> : <span className="sniffer-tile-glyph" aria-hidden="true">•</span>}</div>}
                </div>
              ))}
            </div>
          )}
          ...
          {result.skipped > 0 && <p className="sniffer-warning">{result.skipped} item(s) could not be read and are not included.</p>}
        </section>
      )}
      {contextMenu && (
        <div
        ...
        >
          <div className="sniffer-context-menu-title" title={contextMenu.entry.path}>{contextMenu.entry.name}</div>
          <button type="button" onClick={() => void runContextAction("open")}>
            {contextMenu.entry.isFolder ? "Open folder" : "Open file"}
          </button>
          <button type="button" onClick={() => void runContextAction("reveal")}>Show in Explorer</button>
          <button type="button" onClick={() => void runContextAction("copy-name")}>Copy filename</button>
          <button type="button" onClick={() => void runContextAction("copy-path")}>Copy full path</button>
          <button type="button" onClick={() => void runContextAction("properties")}>Properties</button>
          <div className="sniffer-context-menu-divider" />
          <button type="button" onClick={() => void runContextAction("rename")}>Rename</button>
          <button type="button" className="sniffer-context-danger" onClick={() => void runContextAction("delete")}>Delete</button>
        </div>
      )}
      {toast && <div className="sniffer-toast" role="status">{toast}</div>}
    </main>
  );
}
